...
public class Controller1 : MonoBehaviour
{
    ...
    public async void buttonClick(){
        ...
        tr.action =action;
        tr.reward = reward; 
        tr.next_npc = next_state[0]; 
        tr.next_pc = next_state[1];
        tr.is_done = is_done;


        string data = JsonUtility.ToJson(tr);
        string content =await ApiCall("http://127.0.0.1:8000/dqn/train/",data);
    ...
    async Task DialogueProcess(int episode){
        ...
        fr.episode = episode; 
        string data = JsonUtility.ToJson(fr);
        string content =await ApiCall("http://127.0.0.1:8000/dqn/",data);
       ...
       // Debug.Log("before");
        ...
        string content1 = await ApiCall("http://127.0.0.1:8000/dqn/state/",data1);
        ...
        ar.action = action;
        data = JsonUtility.ToJson(ar);
        
        content = await ApiCall("http://127.0.0.1:8000/dqn/action/", data);
